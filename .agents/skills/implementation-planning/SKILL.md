...
description: >-
  Write a detailed, evidence-backed implementation plan for a firestore-orm
  issue or change into docs/plans/. Use when asked to plan/scope/spec a change,
  break down an issue, or hand work off to another agent (including a Cursor
  Cloud Agent) before any code is written. NOT for writing the code itself, and
