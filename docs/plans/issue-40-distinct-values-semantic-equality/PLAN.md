...
Settled against the issue's own acceptance criteria / "keep the current method documented as
client-side for Core" framing plus the §3 evidence. D5 and D6 are marked `(derived, not asked)`.
**Do not re-litigate any of these** — an implementer who wants a different fork opens a question in
`notes.md` and stops; they do not silently reverse a row.
...
| Plan itself passes the doc/format legs    | `npm run check:docs` with the plan directory present (2026-07-29)                                                  | **clean** — `✓ documentation links OK (181 doc files scanned)`.                                                                                                            |
