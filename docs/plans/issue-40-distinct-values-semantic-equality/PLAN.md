...
**D1-D4 were put to the owner with the §3 evidence and answered by them**; D5 and D6 are marked
`(derived, not asked)`. **Do not re-litigate any of these** — an implementer who wants a different
fork opens a question in `notes.md` and stops; they do not silently reverse a row.
...
| **D3** | The issue also complains `distinctValues()` "downloads all matching docs". Add an internal field-mask projection? | **No — out of scope, deferred to a follow-up issue** ([#75](https://github.com/reggieofarrell/firestore-orm/issues/75)).                                                                                                                                                                                 | *Add `select(field)` internally* — rejected on evidence: the method reads the field as a **literal top-level key** (`doc.data()[field as string]`, `QueryBuilder.ts:1360`), so a model key containing a `.` works today; handing that same string to `select()` makes the SDK read it as a nested path — a silent behavior change. It also saves bytes, not billed document reads. |
...
- **Any field-mask / projection optimization** (D3). Do not touch the raw field read at line **1360**
  or the `hasSelect` guard at **1350-1357**.
...
- **Any other `Set`-based dedupe.** `grep -rn "new Set(" src --include="*.ts" | grep -v "/tests/"`
  returns exactly two hits (**R1**): `QueryBuilder.ts:1364` (this change) and
...
| **R10** | `scripts/check-coverage-gates.mjs:115-119`                                  | The `Pure utilities (src/utils)` gate: `name` 116, `match: file => file.startsWith('src/utils/')` **117**, `thresholds: { lines: 95, branches: 90, functions: 90 }` 118. A **prefix** predicate, so the new file is gated automatically — **no matcher change.** (Contrast: the ADR-0023 footer notes the exact-path matching used elsewhere.) |
...
| `QueryBuilder.ts:1360` (the raw field read) and 1350-1357 (`hasSelect`)   | **D3** (no projection) — and the existing rejection test at `repository-collection-group.integration.test.ts:397-399` pins the guard |
...
- **Do not hardcode the ADR number.** On this baseline `0034` is free (`ls docs/adr/` ends at
  `0033`), but re-check with `ls` before writing (§9.1) — a predecessor merge can claim it.
...
| **I-5** | same, **existing tests at :360, :397, :539**               | Must pass **unchanged** — in particular the stored-`path` carve-out (ADR-0024) and the `select()` rejection                                                                                                  | either regresses         | D3, R2 |
...
merged and this branch was rebased — none is carried over from the `32ce4c1` pass. A second pass on
**2026-07-29** (this handoff) re-executed the probes, greps, §6 compile, unit suite count, unit gate,
and `check:docs`, and corrected residual stale line anchors that the post-#39 rewrite had left in
§2.2 / D3 / §3.6 / I-5 (`1296`/`1300`/`1250-1293` → **1360** / **1364** / **1350-1357**; I-5
`:357/:394/:549` → **`:360/:397/:539`**). The N-ref probe was also taught to honour
`FIRESTORE_EMULATOR_HOST` (it previously hard-coded `8080`, which hung under the plan's own alt-port
recipe).

A **third pass** re-ran all four probes, the §6 compile, and all three doc/format legs, and audited
**every** `file:line` claim in the document against the tree rather than only the ones #39 could have
moved. That found one anchor both earlier passes missed because both were hunting #39-induced drift:
**R10 cited `check-coverage-gates.mjs:110-113`, which was never correct** — the `src/utils` gate is at
**115-119** on `32ce4c1` and `3f0dd7a` alike, and #39 never touched that file. Fixed. The same pass
restored two claims the second pass had weakened: §1's provenance (D1-D4 **were** answered by the
owner, which is what makes the section's title accurate) and this table's format/lint row, whose
`docs/plans`-is-prettier-checked-but-not-linted finding had been dropped. **Lesson for the next
editor: sweeping for the anchors you remember writing is not an audit — check them all.**

| Check                                    | Command / method                                                                                                | Result                                                                                                                                                                    |
| ---------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| §6.1 compiles as written                 | wrote the file to `src/utils/firestoreValueEquality.ts`, ran `npm run test:types`, then removed it               | **clean** — no diagnostics. Reconfirmed 2026-07-29.                                                                                                                        |
| §6.2 call site compiles as written        | scratch `src/_scratch_issue40_qb_import.ts` importing `distinctFirestoreValues` + `ValueAtKey`, returning the exact cast expression; `npm run test:types`; file removed | **clean** — `distinctFirestoreValues(values) as ValueAtKey<Doc,'map'>[]` type-checks. Reconfirmed 2026-07-29. |
...
| Declaration emit                         | `npx tsc --declaration --emitDeclarationOnly --outDir <tmp>`                                                     | `firestoreValueEquality.d.ts` emits `export declare function distinctFirestoreValues<V>(values: Iterable<V>): V[];` and **names no package at all** — no `@google-cloud/firestore` leak. Reconfirmed 2026-07-29. |
| §6.1 passes the lint/format legs          | `npx prettier --check src/utils/firestoreValueEquality.ts`; `npx eslint <same>`                                    | both **exit 0** — the block is copy-verbatim *and* gate-clean. Reconfirmed 2026-07-29.                                                                                      |
| Nominal checks hold on decoded values     | `N-instanceof-across-read-path.mjs` against the emulator, on both an unconverted and a converted read             | **ALL NOMINAL CHECKS HOLD** (exit 0). See §3.3. Reconfirmed 2026-07-29 under `firebase emulators:exec`.                                                                    |
| Algorithm correctness                     | `P-canonical-key-algorithm.mjs`, 27 cases × 3 encodings                                                            | NESTED **0/27 wrong**; `new Set` 9/27; NAIVE 2/27 (exit 0). Found and fixed the **T2** registry bug during planning. Reconfirmed 2026-07-29.                               |
| SDK/emulator semantics                    | `N-sdk-value-shapes.mjs`, `N-ref-equality-and-converter.mjs`                                                       | §3.2's table — all rows are probe output. Reconfirmed 2026-07-29 (N-ref now respects `FIRESTORE_EMULATOR_HOST`).                                                            |
| Every §9 / §10 shell command              | ran each one **on `3f0dd7a`** (re-run 2026-07-29)                                                                 | `grep -rl -- '–#41' docs/adr/` → **12 files** (10 live footers + 2 not-to-sweep); `grep -n "distinctValues" README.md npm-readme.md` → **no rows, exit 1**; `grep -rn "distinctValues\|#40" docs/development/*.md` → **no rows**; `grep -rn "new Set(" src --include="*.ts" \| grep -v /tests/` → **2 rows** (`QueryBuilder.ts:1364`, `dotNotation.ts:32`); `grep -c "distinctValues" src/core/CollectionGroup.ts` → **0**; `grep -n "distinctValues" src/vector/*.ts` → **no rows**; `ls docs/adr/` → ends at `0033`, so `0034` is free; `npm run check:docs` → `✓ … (181 doc files scanned)` with this plan present (180 without). Expected results are stated in §9.4 for the ones that pass by matching nothing. |
| Post-#39 drift re-enumeration             | re-derived every §3 / §6.2 / §9 anchor against `3f0dd7a` rather than assuming                                       | `QueryBuilder.ts` shifted **+64** (import 15→20, `LIMITATION` 1250-1254→1314-1318, `return` 1300→1364); `src/index.ts` +6 (89/91/98); `FirestoreRepository.ts` interface 165→166-225, still **no `query()`**; `reference/query-builder.md` +3 (201-211); `scope-and-capabilities.md` #40 row 56→**57**; `docs/adr/README.md` last row 62→**63**; ADR-0017 amendment anchor 123→**131**, References 148→**156**, issues bullet →**160**. **Unchanged and verified so:** `queries.md:46/297/422/423/430`, `migration-v2-to-v3.md:180`, `repository-query-builder.integration.test.ts:96-124`, `repository-read-only-converter.integration.test.ts:142-158`, `CollectionGroup.ts:155`, `VectorQueryBuilder.ts` method lines, `package.json`, `jest.config.base.js`, `check-coverage-gates.mjs`. Collection-group tests at **360 / 397 / 539**. |
| Plan itself passes the doc/format legs    | `npm run check:format`, `npm run lint`, `npm run check:docs` with the plan directory present (all three re-run 2026-07-29 after the anchor-audit edits) | all three **clean** — `✓ documentation links OK (181 doc files scanned)`. `docs/**/*.md` is *not* prettier-exempt (only `website/**`), so `PLAN.md` and the `.mjs` probes are prettier-checked and pass — re-run this leg after **any** edit to the plan. ESLint, by contrast, **ignores `docs/plans/**` entirely** (`npx eslint <probes dir>` reports "all of the files … are ignored"), so the probes are format-checked but not linted; `npm run lint` passes either way. |
| Baseline suite counts                     | unit suite, clean tree, `3f0dd7a` (re-run 2026-07-29); integration carried from prior measurement on same sha      | unit **31 suites / 383 tests** (reconfirmed); integration **34 suites / 497 tests** (file list still 34; test count not re-run under emulator this pass — same baseline sha) |
| Gate headroom                             | `npm run test:coverage:gate:unit` against on-disk `coverage/unit/lcov.info` (2026-07-29)                            | §3.5 — `src/utils` at 98.91% / 94.67% / 100% — **All unit coverage gates passed.**                                                                                         |
