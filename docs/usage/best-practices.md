...
while cursor pagination jumps directly to the starting position. See [Queries](./queries.md) for the
full pagination API.
...
`bulkUpdate` (or per-document `update`) instead. See [Lifecycle hooks](./lifecycle-hooks.md).
...
[Timestamps](./timestamps.md) for the millisecond converter and the hook-based write conversion
pattern.
...
See [Error handling](./error-handling.md) for the full error taxonomy and
[Troubleshooting](./troubleshooting.md) for index-related tips.
...
See [Transactions](./transactions.md) for the complete transaction-scoped API.
...
See [Lifecycle hooks](./lifecycle-hooks.md) for the full event list and payload shapes.
