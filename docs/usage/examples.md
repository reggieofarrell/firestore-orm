...
[lifecycle hooks](./lifecycle-hooks.md#lifecycle-hooks)), and a service class — so you can see how
the pieces fit together in a real application. For focused explanations of any single mechanism, see
[CRUD operations](./crud-operations.md), the [query builder](./queries.md),
[transactions](./transactions.md), and [error handling](./error-handling.md).
...
cancelling via a [transaction](./transactions.md), and computing revenue with `sum()` / `average()`
[aggregations](./queries.md). Note that every schema declares a required top-level `id: z.string()`,
...
membership in `afterCreate`, and enforces seat limits with a [transaction](./transactions.md) so
...
[query builder guide](./queries.md) for the full surface of `onSnapshot` and cursor pagination.
