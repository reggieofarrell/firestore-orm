...
payload (see [Schema Validation](./schema-validation.md#schema-validation)). `subcollection` takes
the same `writeSchema` option with identical inference. (Converters are not inherited from the
parent repo.)
