...
`subcollection` mirrors [`withSchema`](./schema-validation.md#schema-validation): read/write types
are inferred from schema values, and a trailing options object carries `writeSchema`, `converter`,
and `sentinelPolicy`.
...
[field-value sentinels](./field-value-sentinels.md#per-field-sentinel-approval) for what strict mode
enforces.
...
> [schema validation](./schema-validation.md#schema-validation) for the rules on `id` handling. For
> an **unvalidated** subcollection, construct a repository directly against the full path:
...
is no `.startAfter()` chaining. See [queries](./queries.md) for the full query surface.
...
See [core concepts](./core-concepts.md) for the full converter contract (when `toFirestore` and
...
[`withSchema`](./schema-validation.md#schema-validation) factory over constructing repositories by
hand.

## Related

- [CRUD operations](./crud-operations.md) — create, read, update, delete on the child repository
- [Queries](./queries.md) — filtering, aggregations, pagination, and streaming
- [Schema validation](./schema-validation.md) — required `id`, derived schemas
- [Field-value sentinels](./field-value-sentinels.md) — `sentinelPolicy` and strict mode
- [Core concepts](./core-concepts.md) — repository pattern and converter behavior
