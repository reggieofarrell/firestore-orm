...
1. **The converter is not applied.** The Admin SDK only runs a `FirestoreDataConverter`'s
   ...
   [`createMillisTimestampConverter`](./timestamps.md) exposes as a `number` is still a Firestore
   `Timestamp` on the trigger snapshot.
2. **There is no `id`.** `snapshot.data()` never contains the document id; it lives on
   `snapshot.id`.

A cast silently mislabels both. See [Core Concepts → Firestore Converters](./core-concepts.md) for
the underlying read semantics.
...
required top-level `id`). See [Schema Validation](./schema-validation.md).
